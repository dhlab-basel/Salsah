...
For every salsahObject we have to build one component. Some of them could have sub-components as well.

- emptyObject (object without a  file, metadata only)

- imageObject

- documentObject (object for
...
Every salsah (resource) object needs his own viewer environment (card) with default, but also specific tools.
...
.. figure:: figures/salsahObject_video.png

       video Object (top left) embedded in the sequenceTranscription tool
...
collectionObject (collection / link / book) / salsahCollection
--------------------------------------------------------------
...
**collectionObject/linkObject**

A collection connects various salsahObjects (resources). We can reuse
the salsahCollection component for links and resource annotations as
well. Every user should be able to create a collection and he can share
it with others (share with the whole project team or with single user).

**bookObject** The book is a special collectionObject with a navigation
tool (sth. like timeline) incl. page preview and flick through. The
single page includes the tools from the imageObject.

regionObject (for images and documents)
---------------------------------------

sequenceObject (for video and audio)
------------------------------------
